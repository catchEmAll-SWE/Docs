...
Al fine di una migliore organizzazione sono stati creati 3 repository\textsubscript{G} per: documentazione, PoC\textsubscript{G} e RTB.
\subsubsection{Strutture dei repository\textsubscript{G}}
Di seguito vengono illustrati i contenuti dei repository\textsubscript{G}:
\begin{itemize}
	...
	\begin{itemize}
		\item Script\textsubscript{G} per lo scontorno delle immagini;
		\item Cartella contenente le parti frond-end e back-end del prodotto, sviluppate utilizzando il framework\textsubscript{G} Laravel\textsubscript{G}.
	\end{itemize}
\end{itemize}
...
Devono essere continuamente osservate sia la qualità di processo\textsubscript{G}, per garantire una buona gestione del progetto, sia la qualità di prodotto\textsubscript{G}, per assicurarsi di ottenere un prodotto finale conforme alle richieste del proponente.
...
Questa metrica è di riferimento ai prodotti software ed è utilizzata dal gruppo per verificare che i test svolti sul prodotto finale coprano tutti i requisiti e casi d’uso identificati. Questa metrica è calcolata attraverso il rapporto tra il numero di requisiti e casi d'uso\textsubscript{G} testati e quello di requisiti e casi d'uso\textsubscript{G} totali da testare:
\begin{align*}
	\centering
...
I gradi di accoppiamento sono definiti su una scala di valori indicata qui di seguito:
\begin{itemize}
	\item Message Coupling (livello ottimale);
	\item Data Coupling;
	\item Stamp Coupling;
	\item Control Coupling (accettabile);
...
Sono stati, inoltre, integrati e resi obbligatori nel repository\textsubscript{G} pubblico attraverso le branch protection rules\textsubscript{G} di GitHub\textsubscript{G}.
...
Sarà il responsabile di progetto che avrà la responsabilità di controllare i risultati decidendo se:
\begin{itemize}
...
\begin{itemize}
	\item Il prodotto software sia conforme con i requisiti riportati nel documento Analisi\_dei\_requisiti v 2.0.0;
	\item Dimostrare la correttezza delle attività svolte durante il periodo di verifica.
\end{itemize}
\subsubsection{Validazione della documentazione}
Al termine della stesura di un documento il responsabile si occupa di rileggerlo controllandone anche la correttezza grammaticale, ne calcola l’indice di
Gulpease e si assicura che gli obiettivi stabiliti siano stati raggiunti. Se il documento soddisfa le attese
allora viene approvato per il rilascio, invece se non le soddisfa il responsabile assegna il compito di
sistemare le parti carenti e la procedura si ripete.
